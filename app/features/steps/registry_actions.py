...
def sprawdz_liczbe_kont_w_rejestrze(context, count):
    ...
    assert_equal(ile_kont.json()["count"], count)
...
def usun_konto(context, pesel):
    ...
    assert_equal(message.json(), {"message": "successfuly deleted"})
...
def update_nazwiska(context, pesel, last_name):
    resp = requests.patch(URL + f"/api/accounts/{pesel}", json={"nazwisko": last_name})
    assert_equal(resp.status_code, 201)
